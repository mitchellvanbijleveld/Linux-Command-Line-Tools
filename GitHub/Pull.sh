...
VAR_SCRIPT_CONFIG_FILE="$VAR_BIN_CONFIG_DIR/GitHub/Pull/directories"
...
while IFS= read -r var_repository_line; do
    ...
    if [[ "$(echo "$var_repository_line" | awk '{print $1}')" =~ ^[a-zA-Z0-9]+$ ]]; then
        echoDebug "Assuming the first string is a username..."
        ...
        echoDebug "Repository User : $var_repository_user"
        echoDebug "Repository Dir  : $var_repository_directory"
    else
